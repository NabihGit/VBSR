...
from model.TTSRNet import TTSR
import numpy as np
import cv2
...
if __name__ == '__main__':
    ...
    args.outimg_dir = os.path.join(args.outimg_dir, 'ttsr_img')
    if not os.path.exists(args.outimg_dir):
        os.makedirs(args.outimg_dir)

    args.outpth_dir = os.path.join(args.outpth_dir, 'TTSR')
    ...
    if args.weights:
        if os.path.isfile(args.weights):
            ...
            model_ttsr.load_state_dict(checkpoint)
        else:
            print("=> no checkpoint found at '{}'".format(args.weights))

    print("===> Setting Optimizer")
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model_ttsr.parameters()), lr=args.lr)#SGD()
    ...
    print("===> Training")
    best_weights = copy.deepcopy(model_ttsr.state_dict())
    best_score = 0.0
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        ref_pre = torch.rand(3, 135, 240, dtype=torch.float32).cuda()
        refsr_pre= torch.rand(3, 135 * 4, 240 * 4, dtype=torch.float32).cuda()
        ...
        with tqdm(total=(len(dataset) - len(dataset) % args.batchSize), ncols=80) as t:
            for iteration, (lr_imgs,hr_imgs) in enumerate(dataloader):
                ...
                sr_imgs = model_ttsr.forward_sequence(lr_imgs.unsqueeze(0), ref_pre, refsr_pre).squeeze(0)
                sr_imgs = sr_imgs
                lr_imgs = lr_imgs
                ref_pre = lr_imgs[-1,...]
                refsr_pre = sr_imgs[-1,...]
                if cnt % 10 == 0:
                    ...
                    # concatenate image
                    img_con1 = np.concatenate((lr_img_x4, hr_img, sr_img),axis=1)*255
                    # write image
                    cv2.imwrite(os.path.join(args.outimg_dir, 'ttsr_img_{:09d}.jpg'.format(cnt // 10)), cv2.cvtColor(img_show, cv2.COLOR_BGR2RGB))
                ...
                if cnt % ((len(dataset) - len(dataset) % args.batchSize)/5) == 0:
                    torch.save(model_ttsr.state_dict(), os.path.join(args.outpth_dir, 'model_ttsr_{:04d}.pth'.format(cnt / ((len(dataset) - len(dataset) % args.batchSize)/5))))
        torch.save(model_ttsr.state_dict(), os.path.join(args.outpth_dir, 'model_ttsr_{:04d}.pth'.format(epoch)))
